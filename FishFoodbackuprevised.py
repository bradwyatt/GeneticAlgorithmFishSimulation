...
POPSIZE=10
NUMGENES = 22
SHARKSCORE, scoreBlit, scoreDisappearTimer = 0, 0, 0
highScore = 0
bestGeneration = 0
screen = None
...
class GameOver(object):
    ...
    def main_loop(self):
        ...
        while menuOn == 2:
            ...
            events = pygame.event.get()
            self.screen.blit(self.title, (0, 0))
            scoreGameOverText = arialFont.render("Score: "+str(SHARKSCORE), 1, (0,0,0))
            self.screen.blit(scoreGameOverText, (50, 175))
            if(SHARKSCORE == get_high_score()):
...
def reset():
    global SHARKSCORE, scoreBlit, scoreDisappearTimer
    SHARKSCORE, scoreBlit, scoreDisappearTimer = 0, 0, 0
    keys = [False, False, False, False]
...
sharks = [ Shark() for i in range(10)]
...
while running:
    ...
    if menuOn == 1: #Menu Screen
        ...
        if SHARKSCORE > high_score:
            save_high_score(SHARKSCORE)
    ...
    screen.blit(scoreText, ((screenWidth/2)+200, 5))
    generationText = oceanFont.render("Generation: "+str(generation), 1, (255,255,255))
    screen.blit(generationText, ((screenWidth/2), 5))
    highScoreText = oceanFont.render("Highest Score: "+str(highScore), 1, (255,255,255))
    ...
    for shark in sharks:
        ...
        for redFish in redfishes:
            ...
            if not redFish.stillAlive():
                ...
                arcsList[redFish.arcIndex].image = images["spr_wall"]
